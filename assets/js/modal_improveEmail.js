var improveEmail = {
    paramList: [    //API에 전달할 파라미터 id 목록
        "tone", "quantity", "expression", "spelling_1", "spelling_2", "spelling_3", "mood", "emailBody"
    ],
    selectedRanges: [],
    option_description:[],
    init: function() {
        ...
        document.getElementById("formImproveEmail").querySelectorAll("input").forEach((option) => {
            option.addEventListener("mouseover", improveEmail.showOptionDescription);
    ...
    APIRequest: function() {
        ...
        modal.APIRequest("http://34.22.72.121/correctEmail", params, improveEmail.showResult);
    },
    ...
    showOptionDescription: function(e) {
        ...
        desc = improveEmail.option_description[option][value] ?? improveEmail.option_description[option] ?? null;
        if(desc != null) {
            optionDescriptionArea.children[0].innerText = desc.level + "\n" + desc.description;
    ...
    MaintainDraggedText: function() {
        const editableDiv = document.getElementById('emailBody');
        const highlightDiv = document.getElementById('highlightDiv');        

        editableDiv.addEventListener('input', updateHighlight);
        editableDiv.addEventListener('scroll', syncScroll);
        editableDiv.addEventListener('mouseup', handleSelection);

        function updateHighlight() {
            highlightDiv.textContent = editableDiv.textContent;
            selectedRanges = [];
            applyHighlights();
        }
        function syncScroll() {
            highlightDiv.scrollTop = editableDiv.scrollTop;
            highlightDiv.scrollLeft = editableDiv.scrollLeft;
        }
        ...
        function applyHighlights() {
            ...
            selectedRanges.forEach(range => {
                ...
                highlightedText += `<span class="highlight">${text.substring(range.start, range.end)}</span>`;
                lastIndex = range.end;
            });
